...
function renderCommitments() {
  ...
  elements.commitments.querySelectorAll("[data-interview-slots]").forEach(container => {
    void loadInterviewSlots(container.dataset.interviewSlots, container);
  });
}

async function loadInterviewSlots(id, container) {
  try {
    const result = await api(`/actions/${id}/interview-slots`);
    if (!result.slots.length) {
      container.innerHTML = "<span>No free 30-minute times were found in the next three weeks. Choose a time below.</span>";
      return;
    }
    container.innerHTML = `<strong>Choose a free time</strong><div>${result.slots.map(slot => `
      <button type="button" class="slot-button" data-slot-start="${escapeHtml(slot.start_at)}" data-slot-end="${escapeHtml(slot.end_at)}">
        ${escapeHtml(slot.label)}${slot.preference_match ? " · matches preference" : ""}
      </button>`).join("")}</div>`;
    ...
    container.innerHTML = "<span>Calendar suggestions unavailable. Choose a date and time below.</span>";
  }
}
